...
train_config_77 = ConfigScheme(True, True, False,
...
config_scheme_to_use = train_config_77
...
def gen_train_df(with_hist_profile=True, persist_fe_data=False):
    ...
    train_data, new_features, discretization_bins_used = generate_counting_history_features(train_data,
                                                                                            train_ip_contains_training_day,
...
def gen_test_df(with_hist_profile=True, persist_fe_data=False,
    ...
    if persist_fe_data:
        ...
        train[predictors1 + ['click_id']].to_csv(
            get_dated_filename('test_fe.csv' + '.sample' if use_sample else 'test_fe.csv'), index=False)
