...
categorical = ['app','device',  'os', 'channel','ip','hour']
predictors = ['app','device',  'os', 'channel','ip','hour']
...
test = pd.read_csv(TEST_PATH, dtype=dtypes, usecols=test_columns, parse_dates=["click_time"]).sample(frac=0.5)#.sample(1000)
...
train = pd.read_csv(TRAIN_PATH,
    ...
    parse_dates=["click_time"]).sample(len(test))
timer(t)
...
pprint(sorted(zip(lgb_model.feature_name(),list(lgb_model.feature_importance())),key=lambda x: x[1]))
